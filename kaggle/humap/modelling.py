...
class image_model(keras.Model):
    ...
    '''
    def __init__(self, seed, config, augment=False, augs=['flip'], inspect_augment=True, aug_prop=0.2):
        super().__init__()
        self.seed = seed
        ...
        self.segmenter = UDTransNet(config=config, n_channels=config.num_channels, n_classes=config.num_classes, img_size=config.image_size)
    
    def call(self, images, masks):
        '''
        Augmentation component: Augment (aug_prop*100)% of the images in a given batch
        '''
        num_to_aug = int(images.shape[0] //(1/self.aug_prop))
